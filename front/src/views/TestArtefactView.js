...
function ArtefactView(props) {
    ...
    const StateField = () => (
        <Fragment>
            <TextField
                ...
                onChange={e => handleSetField('state', e.target.value)}
                select
                disabled={edit && !!beingEdited && beingEdited !== 'state'}
            >
                {Object.keys(artefactStates).map(value => {
                    return (
                        <MenuItem
                            value={props.artefactStates[value]}
                            key={value}
                        >
                            {value}
                        </MenuItem>
    ...
    const DescriptionField = () => (
        <Fragment>
            <TextField
                ...
                disabled={
                    edit && !!beingEdited && beingEdited !== 'description'
                }
            />

            {edit && beingEdited === 'description' && <EditButtons />}
        </Fragment>
    )
    ...
    const IsPublicField = () => (
        <Fragment>
            <List
                subheader={
                    <ListSubheader component='div'>Privacy</ListSubheader>
                }
                dense
            >
                <ListItem
                    disabled={
                        edit && !!beingEdited && beingEdited !== 'isPublic'
                    }
                >
                    {!view && (
                        <ListItemIcon>
                            <Checkbox
                                ...
                                tabIndex={-1}
                                onClick={e =>
                                    handleSetField('isPublic', e.target.checked)
                                }
                            />
                        </ListItemIcon>
                    )}
    ...
    const ImageField = () => {
        if (view) {
            return <div>Viewing images not implemented yet</div>
        } else {
            return (
                <DropzoneArea
                    initialFiles={state.files || []}
                    onChange={files => handleSetField('files', files)}
    ...
    const LeftPane = [NameField, StateField, DescriptionField, AdminField]
    const RightPane = [IsPublicField, SelectFamiliesField, ImageField]
    ...
    const AddressField = () => (
        <Fragment>
            <TextField
                ...
                onChange={e => {
                    handleSetField('address', e.target.value)
                    if (e.target.value) {
            ...
            {edit && beingEdited === 'address' && <EditButtons />}
        </Fragment>
    )
